/**
 * split_modules.cjs
 * Splits the overloaded AccountsTab into 3 clean modules:
 *   1. AccountsTab       — account cards only (clean)
 *   2. TransactionsTab   — unified all-outflows ledger (NEW)
 *   3. BalanceLogTab     — periodic balance snapshot + chart (NEW)
 * Also renames "History" → "Monthly Budget" in the nav.
 */
...
// ─── 1. REPLACE AccountsTab (keep only account card management) ───────────────
...
// ─── 2. NEW: TransactionsTab ──────────────────────────────────────────────────

const NEW_TRANSACTIONS_TAB = `// ─── TRANSACTIONS TAB ────────────────────────────────────────────────────────
...
const NEW_BALANCE_LOG_TAB = `// ─── BALANCE LOG TAB ─────────────────────────────────────────────────────────
...
// 1. Replace AccountsTab function
const accTabStart = src.indexOf('function AccountsTab(');
const majorExpStart = src.indexOf('// ─── MAJOR EXPENSES');
if (accTabStart === -1 || majorExpStart === -1) {
  console.error('[ERROR] Could not locate AccountsTab or MajorTab marker');
  process.exit(1);
}
src = src.slice(0, accTabStart) + NEW_ACCOUNTS_TAB + src.slice(majorExpStart);
console.log('[OK] AccountsTab replaced (clean version)');

// 2. Insert TransactionsTab + BalanceLogTab before MajorTab marker
const majorMarker = '// ─── MAJOR EXPENSES';
src = src.replace(majorMarker, NEW_TRANSACTIONS_TAB + NEW_BALANCE_LOG_TAB + majorMarker);
console.log('[OK] TransactionsTab + BalanceLogTab inserted');

// 3. Update NAV_TABS — rename History, add Transactions + Balance Log
const oldNavTabs = `  const NAV_TABS=[
...
const newNavTabs = `  const NAV_TABS=[
    {id:'dashboard',   label:'Dashboard',          icon:'D', group:'main'},
    {id:'accounts',    label:'Accounts',           icon:'A', group:'manage'},
...
if (!src.includes(oldNavTabs)) {
  console.error('[ERROR] NAV_TABS pattern not found — please check spacing');
  // Try partial match
  console.log('Searching for partial match...');
  const idx = src.indexOf("const NAV_TABS=[");
  console.log('NAV_TABS starts at index:', idx);
} else {
  src = src.replace(oldNavTabs, newNavTabs);
...
if (!src.includes(oldTLBL)) {
  console.error('[ERROR] TLBL pattern not found');
} else {
  src = src.replace(oldTLBL, newTLBL);
  console.log('[OK] TLBL updated');
}

// 5. Update TABS (mobile top bar)
...
if (!src.includes(oldTABS)) {
  console.error('[ERROR] TABS pattern not found');
} else {
  src = src.replace(oldTABS, newTABS);
  console.log('[OK] TABS (mobile) updated');
}

// 6. Update Tab Router — add new tabs, simplify AccountsTab call
const oldRouter = `              {tab==='dashboard'&&<Dashboard budgetData={budgetData} accounts={accounts} majorExpenses={majorExpenses} credits={credits} debts={debts} balanceHistory={balanceHistory} sm={sm}/>}
...
const newRouter = `              {tab==='dashboard'   &&<Dashboard budgetData={budgetData} accounts={accounts} majorExpenses={majorExpenses} credits={credits} debts={debts} balanceHistory={balanceHistory} sm={sm}/>}
...
if (!src.includes(oldRouter)) {
  console.error('[ERROR] Router pattern not found');
  // Log a snippet to help debug
  const idx = src.indexOf("tab==='dashboard'");
  if (idx !== -1) {
    console.log('Found dashboard at index:', idx);
    console.log('Surrounding text:\n', src.slice(idx - 20, idx + 200));
  }
} else {
  src = src.replace(oldRouter, newRouter);
  console.log('[OK] Tab router updated');
}

// ─── WRITE OUTPUT ─────────────────────────────────────────────────────────────
fs.writeFileSync(SRC, src, 'utf8');
console.log('\n✅ All done! App.jsx updated successfully.');
console.log('   → AccountsTab: cleaned (account cards only)');
console.log('   → TransactionsTab: new unified ledger module');
console.log('   → BalanceLogTab: new balance snapshot module');
console.log('   → NAV_TABS: updated with new tabs');
console.log('   → Router: updated with new tab routes');
